...
# ============================================================
# zsh_audit  -> health report for the ~/.zsh framework (17 checks)
...
zsh_audit() {
  ...
  if command -v hyperfine >/dev/null 2>&1; then
    ...
    if (( ms < 250 )); then
      ...
      _audit_warn "~$ms ms (target <250 ms; install hyperfine for accurate stats)"
    fi
  fi
  ...
  local dups=0 name
  typeset -A _audit_fname
  for f in "$root"/tools/*.zsh "$root"/tools/*/*.zsh "$root"/lib/*.zsh "$root"/aliases.zsh "$root"/functions.zsh; do
    [[ -f "$f" ]] || continue
    for name in $(grep -oE '^[A-Za-z_][A-Za-z0-9_-]*\(\)' "$f" | sed 's/()//'); do
      if (( ${+_audit_fname[$name]} )); then
        _audit_fail "'$name' defined in multiple modules"
        ((dups++))
      else
        _audit_fname[$name]=1
      fi
  ...
  ((dups == 0)) && _audit_ok "no duplicate definitions"
  unset _audit_fname
  ...
  ((hits == 0)) && _audit_ok "all paths are \$HOME-relative or portable"
  ...
  # ----------------------------------------------------------
  echo
  echo "[16] local.zsh override hook"
  if [[ -f "$rc" ]] && grep -q 'local.zsh' "$rc"; then
    _audit_ok "orchestrator sources \$ZSH_HOME/local.zsh when present"
  else
    _audit_fail "orchestrator has no local.zsh override hook"
    ((rc_failures++))
  fi
  ...
  if ((rc_failures == 0)); then
    print -P "%F{green}== audit passed (17 checks) ==%f"
  else
